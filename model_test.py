...
class RecmModel:
    ...
    def __init__(self, train_data, show_detail):
        ...
        for rate in train_data:
            ...
            index += 1
        self.user_list = list(self.data_udict)
...
class UserBasedModel(RecmModel):
    ...
    def get_sim(self, sim_num=30, work_range=slice(None)):
        index = 0
        Percentage = 0
        len_udict = len(self.data_udict)
        sim_table = {}
        for user_id in self.user_list[work_range]:
    ...
    def get_recm(self, recm_num=15, work_range=slice(None)):
        index = 0
        Percentage = 0
